...
(async () => {
  ...
  while (true) {
    ...
    try {
      ...
      if (!submitClicked && /Timeout/i.test(e.message) && softFails < 6) {
        ...
        if (!DRY) saveState(state);
        log(`일시적인 지연이라 30초 뒤 다시 시도해요 (${softFails}/6): ${String(e.message).split(String.fromCharCode(10))[0].slice(0, 70)}`);
        await sleep(30 * 1000);
        continue;
      }
