...
function loadState() {
  ...
  s.lastAt = s.lastAt || {};
  s.skipIds = s.skipIds || [];      // 오늘 건너뛸 글 번호
  s.badBoards = s.badBoards || [];  // 오늘 글쓰기 화면이 안 열린 게시판 ('카페|게시판')
  s.fails = s.fails || {};
  return s;
}
...
async function unusedFor(cafe, state) {
  const res = await fetch(API);
  const all = await res.json();
  const skipIds = (state && state.skipIds) || [];
  const badBoards = (state && state.badBoards) || [];
  return all
    .filter((x) => !x.used && x.cafe === cafe && !skipIds.includes(x.id) && !badBoards.includes(`${cafe}|${x.category}`))
    .sort((a, b) => a.id - b.id);
}
...
async function postOne(page, item, state) {
  ...
  // 게시판 목록이 늦게 그려질 수 있어서 최대 3번, 목록을 다시 열어가며 찾음
  let boardPicked = false;
  for (let k = 0; k < 3 && !boardPicked; k++) {
    if (k > 0) { await page.keyboard.press('Escape').catch(() => {}); await page.waitForTimeout(1500); }
    await sel.nth(2).click();
    const boardOpt = page.locator('.n-base-select-option:visible').filter({ hasText: item.category }).first();
    const found = await boardOpt.waitFor({ timeout: 6000 }).then(() => true).catch(() => false);
    if (found) { await boardOpt.click(); boardPicked = true; }
  }
  if (!boardPicked) throw new Error(`게시판 '${item.category}' 을(를) 목록에서 못 찾았어요 (${cafe})`);
  await page.waitForTimeout(2000);

  // 게시판에 따라 글쓰기 화면(본문 입력칸)이 안 열리는 경우가 있어 먼저 확인
  const editorReady = await page.getByText('내용을 입력하세요').first().waitFor({ timeout: 12000 }).then(() => true).catch(() => false);
  if (!editorReady) {
    const tokenErr = (await page.getByText('에디터 토큰').count()) > 0;
    throw new Error(`BOARD_UNAVAILABLE:${item.category}:${tokenErr ? '에디터 토큰 오류' : '본문 입력칸 없음'}`);
  }
  ...
  if (c && c.comment) {
    ...
    if (c.reply) {
      ...
      await page.waitForTimeout(1200);
      const rbox = page.locator('.reply-comment-register').first(); // 답글 전용 칸 (일반 댓글 칸 create-comment-register에 넣으면 대댓글이 아니라 새 댓글이 됨)
      if (!(await rbox.count())) throw new Error('답글 입력칸을 찾지 못했어요');
      ...
      await page.waitForTimeout(1200);
      const nested = await page.evaluate((txt) => {
        const hit = [...document.querySelectorAll('.comment-container')].filter((c) => c.innerText.includes(txt));
        return hit.length ? parseFloat(getComputedStyle(hit[hit.length - 1]).marginLeft) : -1;
      }, c.reply.slice(0, 15));
      if (!(nested > 0)) throw new Error(`대댓글이 답글(안쪽)로 들어가지 않았어요 (들여쓰기 ${nested})`);
      await page.waitForTimeout(1500);
      log(`대댓글: ${au.text}`);
    }
  }
...
(async () => {
  ...
  let closedRetries = 0;
  let softFails = 0;
  const state = DRY ? { date: today(), done: {}, lastAt: {}, nickIdx: {}, cmtIdx: {}, skipIds: [], badBoards: [], fails: {} } : loadState();
  let count = 0;
  ...
  while (true) {
    ...
    if (DRY) {
      ...
      if (!item) for (const cafe of CAFES) { const list = await unusedFor(cafe, state); if (list.length) { item = list[0]; break; } }
      ...
      const list = await unusedFor(next.c, state);
      if (!list.length) {
    ...
    try {
      ...
      if (String(e.message).startsWith('BOARD_UNAVAILABLE')) {
        // 그 게시판은 글쓰기 화면이 안 열림 → 오늘은 그 게시판 글을 건너뛰고 다른 글로 계속
        const [, cat, why] = e.message.split(':');
        const key = `${item.cafe}|${cat}`;
        if (!state.badBoards.includes(key)) state.badBoards.push(key);
        if (!DRY) saveState(state);
        log(`'${cat}' 게시판은 글쓰기 화면이 열리지 않아요 (${why}). 오늘은 이 게시판 글을 건너뛰고 다른 글로 계속해요.`);
        if (DRY) break;
        continue;
      }
      if (!submitClicked && !/로그인이 풀렸/.test(e.message) && !/closed|Target/i.test(e.message) && softFails < 6) {
        // 글이 올라가기 전의 일시적 지연: 같은 글은 두 번까지만 다시 시도하고 그래도 안 되면 그 글은 건너뜀
        softFails++;
        state.fails[item.id] = (state.fails[item.id] || 0) + 1;
        if (state.fails[item.id] >= 2 && !state.skipIds.includes(item.id)) { state.skipIds.push(item.id); log(`글 #${item.id}은(는) 두 번 실패해서 건너뛰어요`); }
        if (!DRY) saveState(state);
        log(`글이 올라가기 전에 문제가 생겨 30초 뒤 다시 시도해요 (${softFails}/6): ${String(e.message).split(String.fromCharCode(10))[0].slice(0, 70)}`);
        await sleep(30 * 1000);
        continue;
      }
